...
// ─── deterministyczny pseudo-random (identyczny z mockAssets.ts) ─────────────
...
// ─── generatory ──────────────────────────────────────────────────────────────
function genSN(seed: number): string {
...
function generateAssets(): AssetData[] {
  ...
  const batches: [string, ModelDef[], number][] = [
  ...
  for (const [category, models, count] of batches) {
    for (let i = 0; i < count; i++) {
      ...
      assets.push({
        id, assetName, assetTag, serialNumber: sn,
        manufacturer: model.manufacturer,
        modelName: model.modelName,
        modelNumber: model.modelNumber,
        category, deviceRole: model.role,
        assignedTo: person, location, zone, status,
        lastUpdate: date,
        ...extra,
      });
    }
...
async function main() {
  ...
  // Usuń stare dane żeby uniknąć konfliktów
  ...
  await prisma.user.createMany({
    data: PEOPLE.map((name, i) => ({
      name,
      email: `${name.toLowerCase().replace(/\s+/g, '.').replace(/[ąćęłńóśźż]/g, c =>
        ({ ą:'a',ć:'c',ę:'e',ł:'l',ń:'n',ó:'o',ś:'s',ź:'z',ż:'z' }[c] ?? c)
      )}@labelmill.com`,
      department: i < 3 ? 'IT' : i < 8 ? 'Office' : 'Production',
    })),
  });
